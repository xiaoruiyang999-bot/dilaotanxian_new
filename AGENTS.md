...
12. **固定特殊房门禁**：先在地图层约束叶子/跨格/入口/距离，再套唯一模板；Boss 仪式厅固定 2×2、南入口，仅房间位置可随机，房内 Plan/地墙/装饰/内容不得消费地图 seed，墙朝向只读本房局部拓扑；完整门宽必须落在真实相邻粗格内（v1.1.52）。
13. 文档惯例：功能/修复落盘配 `v0.x.y_*.md`；教训回写 `开发必读_核心信息整合.md`；里程碑总账在《项目长期计划_Roguelite路线图》。
