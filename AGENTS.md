...
17. **统一暂停与模态输入**：阻塞型 UI 只通过 GameModalService 令牌进入/退出；禁止各面板直接写 Time.timeScale。Esc 只交给顶层模态，关闭最后一层后同帧仍屏蔽世界输入；Hit Stop 不得恢复仍被 UI 暂停的世界。
18. **新局与续局分流**：准备房传送门固定开始新 Run，并在进地牢前同步清理旧 ActiveRun；续局必须使用未来独立的显式入口。死亡返回、暂停菜单重开等结束本局的路径必须在切场景前同步删除 ActiveRun，不能把清理寄托给可能随场景卸载中断的协程。
...
- 从 v2.1.2 起，每个后续小版本实施前须在《计划书》新增 `vX.Y.Z_主题_计划书.md`；实施中维护同目录 `vX.Y.Z_问题汇总与修复记录.md`，按症状、证据、根因、处理、验证和遗留项记录，即使无问题也写明“未发现”。版本完成前两份文件都必须存在，根目录同版本实施记录只写实际结果，不替代计划书。
