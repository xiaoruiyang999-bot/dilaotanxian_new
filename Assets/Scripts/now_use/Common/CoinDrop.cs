...
/// <summary>
/// 金币掉落物（v1.1.3 自 MCP 分支还原，M2·v0.7.0 原型）：敌人死亡/宝箱散落的金币。
/// 无刚体无碰撞体（不参与物理世界、不弹跳、不被墙挡）——散落与磁吸全部代码驱动：
/// 随机方向滑散减速停下 → 靠近玩家磁吸加速 → 触碰入账（PlayerStats.AddCoins）。
/// 零资产纯代码（程序员美术）：金色小圆 = 程序生成 Sprite，静态缓存共享。
/// 与 ManaOrb 同款挂载策略（场景根 + 自清理）：主分支 dungeonRoot 为场景接线私有字段无固定名，
/// 故不寻父，12s 生命周期兜底防楼层切换/场景残留泄漏。
/// </summary>
public class CoinDrop : MonoBehaviour
{
    ...
    private const float SlideDamping = 0.90f;   // 滑散速度衰减基数（按 dt×60 帧率无关化）
    ...
    void Awake()
    {
        ...
        sr.sortingOrder = 4;   // 法力球(1/2)之上，玩家(10)/武器(11)之下
