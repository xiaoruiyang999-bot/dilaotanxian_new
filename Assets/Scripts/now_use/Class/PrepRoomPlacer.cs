...
public static class PrepRoomPlacer
{
    ...
    /// <summary>以 center 为职业角色选择台位置生成三展台（武器展示台左右各 2.6）。</summary>
    public static void Spawn(Vector3 center, Transform parent)
    {
        classPedestal = PrepPedestal.Create(PrepPedestalType.CharacterSelector, center, parent);
        ...
        Debug.Log("[Run] 准备房间：三展台已生成（职业角色选择台 + 兼容武器展示台 ×2）");
    }

    /// <summary>确认职业角色后刷新两个武器展台，兼容池以角色定义为唯一真值。</summary>
    public static void RefreshWeapons(PlayableCharacterDefinition definition)
    {
        if (definition == null || definition.AvailableWeapons.Count == 0) return;

        shownL = definition.AvailableWeapons[0];
        shownR = definition.AvailableWeapons.Count > 1 ? definition.AvailableWeapons[1] : null;

        if (weaponPedestalL != null) weaponPedestalL.ShowWeapon(shownL);
        if (weaponPedestalR != null) weaponPedestalR.ShowWeapon(shownR);

        Debug.Log($"[Run] 武器展台已刷新：{definition.DisplayName} 兼容武器");
    }
